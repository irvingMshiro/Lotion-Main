    ...
    return (
        <div className="text-sm my-3">
            ...
            <div className="mt-2 px-1">
                <p className="hidden last:block text-xs text-center align-middle text-muted-foreground">
                    Your trash is empty
                </p>
                {filteredDocuments?.map((document) => (
                    <div key={document._id} role="button" onClick={() => onClick(document._id)}
                        className="text-sm rounded-sm w-full hover:bg-primary/5 flex items-center text-primary justify-between  pb-1">
                        <span className="truncate pl-2">                   
                            {document.title}                           {/*    returns the document title of everything in getTrash   */}
                        </span>
